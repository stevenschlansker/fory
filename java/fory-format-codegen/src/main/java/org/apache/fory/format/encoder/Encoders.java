...
public class Encoders {
  ...
  static void loadMapCodecs(TypeRef<?> type, CodecEncoding codecFactory) {
  ...
  private static TypeRef<?> token4BeanLoad(
      Set<TypeRef<?>> set, TypeRef<?> init, CodecEncoding codecFactory) {
  ...
  static Class<?> loadOrGenRowCodecClass(Class<?> beanClass, CodecEncoding codecFactory) {
    Set<Class<?>> classes =
   ...
   * for the same bean.
   */
  static String stableQualifiedCodecName(
      Class<?> beanClass, String prefix, CodecEncoding codecFactory) {
